...
app.post('/contest/:id/edit', async (req, res) => {
  try {
    ...
    if (!contest) {
      ...
      if (!['noi', 'ioi', 'acm', 'goi'].includes(req.body.type)) throw new ErrorMessage('无效的赛制。');
  ...
  try {
    ...
    if (player) {
      for (let problem of problems) {
        ...
        } else if (contest.type === 'ioi' || contest.type === 'goi') {
          if (player.score_details[problem.problem.id]) {
            let judge_state = await JudgeState.findById(player.score_details[problem.problem.id].judge_id);
            problem.status = judge_state.status;
            problem.judge_id = player.score_details[problem.problem.id].judge_id;
    ...
    if ((!contest.hide_statistics) || (contest.ended) || (isSupervisior)) {
      ...
      for (let problem of problems) {
        ...
        if (contest.type === 'ioi' || contest.type === 'noi' || contest.type === 'goi') {
        ...
        for (let player of players) {
          if (player.score_details[problem.problem.id]) {
            ...
            if ((contest.type === 'acm' && player.score_details[problem.problem.id].accepted) || ((contest.type === 'noi' || contest.type === 'ioi' || contest.type === 'goi') && player.score_details[problem.problem.id].score === 100)) {
              problem.statistics.accepted++;
            }

            if ((contest.type === 'noi' || contest.type === 'ioi' || contest.type === 'goi') && player.score_details[problem.problem.id].score > 0) {
